...
async function buildShiprocketPayload(
  ...
  // Collect total weight and dimensions (using first item's dimensions for simplicity)
  let totalWeight = 0;
  let totalLength = 0;
  let totalWidth = 0;
  let totalHeight = 0;
  ...
  for (const item of itemsWithVariants) {
    ...
    totalHeight += dims.height * item.quantity; // stacking height approximation
